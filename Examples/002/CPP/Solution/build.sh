...
CXX=mpicxx cmake -B build_profile -DCMAKE_BUILD_TYPE=Debug
# Compile the profile version
cmake --build build_profile

ldd build_profile/mandelbrot_mpi
...
SCOREP_WRAPPER=off cmake -B build_scorep -DCMAKE_BUILD_TYPE=Debug \
    -DCMAKE_CXX_COMPILER=${SCOREP_MPICXX} \
    -DCMAKE_C_COMPILER=${SCOREP_MPICC} 
    
# Compile it
cmake --build build_scorep

ldd build_scorep/mandelbrot_mpi
