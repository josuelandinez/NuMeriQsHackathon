...
export PROGRAM="${BUILD_DIR}/aos_soa"
...
# CALL TREE: Shows who called who (useful for tracking down where 'random' was called from)
perf report -i ${PERF_DATA_FILE} --stdio -g graph,0.5,caller > ${PERF_DIR}/report_hotspots_tree.txt

# ANNOTATED SOURCE/ASSEMBLY: Shows where time is spent line-by-line
perf annotate -i ${PERF_DATA_FILE} --stdio > ${PERF_DIR}/report_annotate.txt

# RAW STACKS (Useful for advanced visualizations)
perf script -i ${PERF_DATA_FILE} > ${PERF_DIR}/stacks.perf
